...
// export type UserTitle = 'Dr.'| 'Prof.'| null;
...
//   return possibleTitles.includes(suspectedUser?.title) && typeof suspectedUser?.firstName === 'string' && typeof suspectedUser?.lastName === 'string' && typeof suspectedUser?.joinedAt === 'number';
 ...
 * Builds a User from a SimpleUser
 * 
 * @param {User} info A User
 * @returns {IdentifiedUser} An IdentifiedUser with the same attributes as prototype, with a current timestamp for the additional 'joinedAt'-attribute
 */
export function buildIdentifiedFromUser(id: number, info: User): IdentifiedUser{
  if(!isUser(info)){
    throw new Error("buildUser: Expected SimpleUser-object");
  }
  // return {id: id, title: info.title, firstName: info.firstName, lastName: info.lastName};
  return Object.assign({id: id}, simplifyUser(info));
}
